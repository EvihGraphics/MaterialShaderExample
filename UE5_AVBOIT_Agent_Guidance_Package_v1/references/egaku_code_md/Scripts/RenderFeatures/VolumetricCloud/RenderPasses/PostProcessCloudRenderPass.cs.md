# Egaku source: `Scripts/RenderFeatures/VolumetricCloud/RenderPasses/PostProcessCloudRenderPass.cs`
...
namespace RenderFeatures.VolumetricCloud.RenderPasses {
	...
	public class PostProcessCloudRenderPass : ScriptableRenderPass {
		...
		private Dictionary<int, VolumetricCloudRenderFeatureLegacy.PerCameraRenderContext> _perCameraContexts = new();
		...
		private VolumetricCloudRenderFeatureLegacy.PerCameraRenderContext GetPerCameraRenderContext(Camera camera,
			in RenderTextureDescriptor textureDescriptor) {

			VolumetricCloudRenderFeatureLegacy.PerCameraRenderContext renderContext;
			if (false == _perCameraContexts.TryGetValue(camera.GetInstanceID(), out renderContext)) {
				renderContext = new VolumetricCloudRenderFeatureLegacy.PerCameraRenderContext(textureDescriptor, 2,_checkerboardRendering);
		...
		public override void Execute(ScriptableRenderContext ctx, ref RenderingData data) {
			...
			VolumetricCloudRenderFeatureLegacy.PerCameraRenderContext cameraCtx = GetPerCameraRenderContext(camera, textureDescriptor);

# Egaku source: `Shaders/OceanLegacy/ShaderLabs/TextureTilingRandomization.hlsl`
...
float2 RandomScaleOffsetUV(float2 uv, float scaleMin, float scaleMax, float3 random3)
{
    ...
    float angle = hash.z * (0.2f * 3.14159265f);
    float rx = cos(angle) * scale;
    float ry = sin(angle) * scale;
...
void RandomScaleOffsetUV_float(float2 uv, float scaleMin, float scaleMax, float3 random3, out float2 outUV)
{
    outUV = RandomScaleOffsetUV(uv, scaleMin, scaleMax, random3);
}
...
TriangleFloat4 SampleTriangleNormalTextures(TEXTURE2D_PARAM(textureName, samplerName), TriangleUV uv, float NormalScale)
{
    TriangleFloat4 result;
    result.r = SAMPLE_TEXTURE2D(textureName, samplerName, uv.r);
    result.g = SAMPLE_TEXTURE2D(textureName, samplerName, uv.g);

    result.r = normalize(result.r * NormalScale);
    result.g = normalize(result.g * NormalScale);

    result.b.xyz = normalize(cross(result.r.xyz, result.g.xyz));
...
float4 SampleTextureRandomizedHexGridMM(TEXTURE2D_PARAM(textureName, samplerName), float2 uv, float hexSize, float edgeContrast, float ScaleMin, float ScaleMax)
{
    ...
    TriangleUV triUV;
    triUV.r = RandomScaleOffsetUV(uv, ScaleMin, ScaleMax, GridRandom3(tri.vertexCoords.r));
    triUV.g = RandomScaleOffsetUV(uv, ScaleMin, ScaleMax, GridRandom3(tri.vertexCoords.g));
    triUV.b = RandomScaleOffsetUV(uv, ScaleMin, ScaleMax, GridRandom3(tri.vertexCoords.b));

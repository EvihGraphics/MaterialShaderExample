# Egaku source: `Shaders/OceanLegacy/ShaderLabs/BakedOceanWave.shader`

> 自动从 `EgakuRenderPipeline.zip` 的文本源码转译。该实现是 Unity 参考，不决定 UE5 架构。

```hlsl
Shader "Custom/BakedSeaWave"
{
    Properties
    {
	...
	float2 GetTerrainMapUV(float3 positionWS,TerrainParams params)
	{
		float2 uvScale = 1.0f / float2(params.mapSizeU,params.mapSizeV);
		float3 posVec =  positionWS - params.axisOrigin;
	...
	WaveData GetBasicWaveData(TerrainParams terrainParams,NearshoreWaveParams nearWaveParams,DistantWaveParams distWaveParams,float3 positionWS)
	{
		WaveData waveData;
		float3 nearshorePosWS = positionWS;
		float2 terrainUV = GetTerrainMapUV(nearshorePosWS,terrainParams);
		...
		waveData.normalWS = float3(0,1,0);
		waveData.velocity =  float4(nearshoreResult.velocity,distResult.velocity);
		waveData.positionWS = positionWS + lerp(nearshoreResult.positionDisplacement,distResult.positionDisplacement,nearDistMask.y);
	...
	void MergePositionWithDetails(FoamMapData foamMaskData,DetailWaveData detailWaveData,inout WaveData waveData){
		...
		waveData.positionWS += lerp(float3(0,0,0),detailWaveData.positionDisplacement,waveData.nearDistMask.y);
		waveData.positionWS += foamMaskData.positionDisplacement*waveData.foamMask;
	}
	...
	Varyings vert_wave(Attributes a2v)
	{
		...
		waveData.normalWS = normalize(cross(neighborWaveData1.positionWS - waveData.positionWS,
			neighborWaveData2.positionWS - waveData.positionWS));
		...
		v2f.normalWS = waveData.normalWS;
		v2f.positionWS = waveData.positionWS;

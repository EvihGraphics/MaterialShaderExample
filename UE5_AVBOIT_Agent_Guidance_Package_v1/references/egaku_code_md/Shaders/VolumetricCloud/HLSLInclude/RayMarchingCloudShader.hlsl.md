# Egaku source: `Shaders/VolumetricCloud/HLSLInclude/RayMarchingCloudShader.hlsl`
...
float3 TransmittancePointToLight(CloudDesc cloudDesc, SunLightDesc lightDesc, float3 litPoint, int iteration) {
    ...
    tr = BeerLambertTransmittance(accumulatedDensity * absorptionLight);
...
float4 ShadeVolumetricCloud(CloudDesc cloudDesc, RayMarchingDesc rayMarchDesc, SunLightDesc lightDesc) {
    BoundingBox bounds = cloudDesc.bounds;
    ...
    */
    ...
    for (int i = 0; i < 100; i++) {
      ...
      //  float3 deltaLightEnergy = TransmittancePointToLight(cloudDesc,  lightDesc, realSamplePoint, rayMarchDesc.scatterMarchIteration) * currStepSize;
        ...
        transmittanceToEye *= BeerLambertTransmittance(sampledDensity *  cloudDesc.absorptionLight * currStepSize);
...
float4 BlendWithBackground(float4 backgroundColor, float3 cloudColor, float transmittance)
{
...
float4 ShadeCloud(in CloudDesc cloudDesc, in RayMarchingDesc rayMarchDesc, in SunLightDesc lightDesc,in half4 backgroundColor) {
    ...
    float tr = BeerLambertTransmittance( result.a * cloudDesc.absorptionScatter.r);

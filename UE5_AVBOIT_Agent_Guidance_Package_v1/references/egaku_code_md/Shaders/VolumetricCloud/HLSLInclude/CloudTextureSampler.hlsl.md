# Egaku source: `Shaders/VolumetricCloud/HLSLInclude/CloudTextureSampler.hlsl`
...
//    float shapeFBM = dot(shapeNoise, normalizedShapeWeights) * heightGradient;
  ...
  //  float density = saturate(dot(general,1) - dot(detail,1) - dot(weather,1));
  //  density *= densityScale;
  //  density = clamp(density, 0, 1);

    float4 cloudVolume = tex3D(cloudTex, cloudUVW)*cloudValueMulti + cloudValueAdd;
    float fbm = dot(cloudVolume.gba,1);
    float density = remap(cloudVolume.r, -(1 - fbm), 1, 0, 1);
   // density = cloudVolume.r - fbm;
    density = saturate(density - weather.r)*densityScale;
   // return density;
    float factor = 0.2;

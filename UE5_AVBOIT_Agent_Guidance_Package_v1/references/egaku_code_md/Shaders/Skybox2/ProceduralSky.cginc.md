# Egaku source: `Shaders/Skybox2/ProceduralSky.cginc`
...
void vert_sky( inout v2f o)
{
    ...
    if(o.rayDir.y >= 0.0)
    {
        ...
        far = sqrt(kOuterRadius2 + kInnerRadius2 * o.rayDir.y * o.rayDir.y - kInnerRadius2) - kInnerRadius * o.rayDir.y;

        float3 pos = cameraPos + far * o.rayDir;

        // Calculate the ray's starting position, then calculate its scattering offset
        float height = kInnerRadius + kCameraHeight;
        float depth = exp(kScaleOverScaleDepth * (-kCameraHeight));
        float startAngle = dot(o.rayDir, cameraPos) / height;
        ...
        float3 sampleRay = o.rayDir * sampleLength;
...
//				for(int i=0; i<int(kSamples); i++)
        {
            ...
            float cameraAngle = dot(o.rayDir, samplePoint) / height;
    ...
    {
        // Ground
        far = (-kCameraHeight) / (min(-0.001, o.rayDir.y));

        float3 pos = cameraPos + far * o.rayDir;

        // Calculate the ray's starting position, then calculate its scattering offset
        float depth = exp((-kCameraHeight) * (1.0/kScaleDepth));
        float cameraAngle = dot(-o.rayDir, pos);
	...
	o.skyColor	= _Exposure * (cIn * getRayleighPhase(_WorldSpaceLightPos0.xyz, -o.rayDir));
	...
	half calcSunSpot(half3 vec1, half3 vec2)
	{
		...
		half spot = 1.0 - smoothstep(-0.2f, _SunSize, dist);

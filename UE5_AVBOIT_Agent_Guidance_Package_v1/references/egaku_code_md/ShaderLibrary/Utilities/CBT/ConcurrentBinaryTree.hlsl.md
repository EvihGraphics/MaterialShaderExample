# Egaku source: `ShaderLibrary/Utilities/CBT/ConcurrentBinaryTree.hlsl`
...
*/
#pragma once
#ifndef CONCURRENT_BINARY_TREE_HLSL
#define CONCURRENT_BINARY_TREE_HLSL

#include "CBTBufferDeclaration.hlsl"
...
void cbt__SetBitValue(uint bufferID, uint bitID, uint bitValue)
{
    ...
    InterlockedAnd(GetCBTBuffer()[bufferID], bitMask);
    InterlockedOr(GetCBTBuffer()[bufferID], bitValue << bitID);
}
...
cbt__BitFieldInsert(uint bufferID, uint bitOffset, uint bitCount, uint bitData)
{
    ...
    InterlockedAnd(GetCBTBuffer()[bufferID], bitMask);
    InterlockedOr(GetCBTBuffer()[bufferID], bitData << bitOffset);
}
...
{
    cbt__HeapArgs args = cbt__CreateHeapArgs(node, bitCount);
    uint lsb = cbt__BitFieldExtract(GetCBTBuffer()[args.heapIndexLSB],
                                    args.bitOffsetLSB,
                                    args.bitCountLSB);
    uint msb = cbt__BitFieldExtract(GetCBTBuffer()[args.heapIndexMSB],
                                    0u,
                                    args.bitCountMSB);
...
uint cbt__HeapRead_BitField(in const cbt_Node node)
{
    ...
    return cbt__GetBitValue(GetCBTBuffer()[bitID >> 5u], bitID & 31u);
}
...
int cbt_MaxDepth()
{
    return firstbitlow(GetCBTBuffer()[0]);
}
